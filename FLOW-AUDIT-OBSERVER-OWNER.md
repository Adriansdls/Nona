...
3. **✅ DONE — web/observer polish batch** (verified via Playwright + tsc clean): B3 "Quando viste?" time selector → maps to observedTimeConfidence/Source (was hardcoded now/exact); `isOwner` derived from VERIFIED ownerData (no flicker, no bogus-token chrome); map shows GPS-less sightings on a centroid offset flagged "zona aprox." (was dropping them); intake/stream now emails the owner their private `?t=` link (guarded against the noreply default); FB-boost hidden unless a real fb_post_id exists; `NonaNav` gets locale on the avistamento page; `_sobre_markup` KeyError guard.
4. **Still pending (sim-safe, needs schema/lib or lower value)**: `_new_sighting_sweep` (needs a sightings claim column + migration — deferred to avoid a half-correct poll); GPS on bot sightings (needs sim_volunteers home_coords lookup); real QR render (needs client QR lib — currently PDF has the real QR); homepage `activeAgentsCount` fake; httpx async in maps/broadcast; failed-send retry cap.
