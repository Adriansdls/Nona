...
- artifact_dir: research/runs/dog-movement-algarve-20260528T072158Z-1698581e
- temp_dir: research/runs/dog-movement-algarve-20260528T072158Z-1698581e/temp
- claims_dir: research/runs/dog-movement-algarve-20260528T072158Z-1698581e/temp/claims
- query_file_path: research/runs/dog-movement-algarve-20260528T072158Z-1698581e/query.md
- final_report_path: research/notes/final_report_dog-movement-algarve.md
- modality: synthesize
- forced_tier: light (user command arg)

## Modality Classification

**synthesize** — The brief asks for integration of multiple empirical domains (movement ecology, scent physics, veterinary heat stress, lost pet recovery statistics, regional climatology) into actionable parameters for a probability model. Not a compare (no ranked options) or collect (not enumerative listing). Synthesize with defended quantitative claims is the right frame.

## Tier Rationale

User explicitly passed `light` as command argument. Confirmed appropriate: the query has 5 well-bounded sub-topics each with specific empirical questions (terrain corridors, scent physics, water gravity, vehicle transport, heat cycles). There is no contested theoretical claim requiring adversarial depth investigation. The output is a structured parameter reference for a probability model — not a defended thesis. Response_format=structured. Light tier (steps 1→2→10→15→16) is the correct tier; full dialectical critics and patcher would add cost without improving a structured parameter synthesis. Citation_style=wikilink (default, no wrapper override).

## Wrapper Requirements

- Output format: per sub-topic with 4 sections each (Physical/empirical principles, Algarve-specific factors, Actionable parameters, Search strategy implications)
- Save path: research/notes/final_report_dog-movement-algarve.md
- Citation format: not specified — use inline citations with source names
- Key sources: Kat Albrecht / Lost Pet Research & Recovery, SAR scent literature, movement ecology (domestic animals), vet heat stress literature, IPMA/OSM Algarve data

## Step Todos

- [ ] Step 1 — hyperresearch-1-decompose
- [ ] Step 2 — hyperresearch-2-width-sweep
- [ ] Step 10 — hyperresearch-10-triple-draft (light: single draft)
- [ ] Step 15 — hyperresearch-15-polish
- [ ] Step 16 — hyperresearch-16-readability-audit
