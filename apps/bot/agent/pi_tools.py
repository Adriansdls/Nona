...
# ---------------------------------------------------------------------------
# WS1 — Guided step sequencer (SINGLE SOURCE OF TRUTH for step order).
#
# The web mirror lives at apps/web/src/lib/guided/sequencer.ts and is used ONLY
# for the HandoffCard preview + web fallback. A parity test
# (apps/bot/tests/test_sequencer_parity.py) asserts both produce identical step
# titles for the same (bucket, is_hard). If you edit STEP_TITLES / WAIT_STEPS
# here, edit the web mirror too.
# ---------------------------------------------------------------------------

GUIDE_BUCKETS = ("h0_6", "h6_24", "d2_4", "d5_10", "d10_plus")
...
WAIT_STEPS: dict[str, dict[str, str]] = {
    "h0_6": {
...
def bucket_from_hours(hours: float) -> str:
    if hours < 6:
        return "h0_6"
    if hours < 24:
        return "h6_24"
...
def build_step_sequence(bucket: str, is_hard: bool) -> list[dict[str, Any]]:
    """
    Ordered single-step sequence for a case. Hard/passive cases get the action
    steps + a designed WAIT step; soft cases get the action steps only.
    Each step: { idx, kind: 'action'|'wait', title, why? }.
    """
    titles = STEP_TITLES.get(bucket, STEP_TITLES["h0_6"])
    steps: list[dict[str, Any]] = [
        {"idx": i, "kind": "action", "title": t} for i, t in enumerate(titles)
    ]
    if is_hard:
        w = WAIT_STEPS.get(bucket, WAIT_STEPS["h0_6"])
        steps.append({"idx": len(steps), "kind": "wait", "title": w["title"], "why": w["why"]})
    return steps


def step_titles(bucket: str, is_hard: bool) -> list[str]:
    """Titles only — used by the parity test against the web mirror."""
    return [s["title"] for s in build_step_sequence(bucket, is_hard)]


def sequence_for_case(case: dict[str, Any], hours_elapsed: float) -> list[dict[str, Any]]:
    """Derive (bucket, is_hard) from the case's behavioral_profile + elapsed hours."""
    bp = case.get("behavioral_profile") or {}
    gate = bp.get("action_gate") or {}
    # is_hard = passive profile: active search not permitted (mirrors compute_action_gate).
    is_hard = (not gate.get("active_search_permitted", True)) or bool(gate.get("crowd_response_blocked"))
    return build_step_sequence(bucket_from_hours(hours_elapsed), is_hard)
