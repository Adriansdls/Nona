...
1. **Dynamic alert radius, not a fixed 10km.** ✅ DONE 2026-05-31 (`harness.compute_alert_radius_km` → `request_volunteer_alert`). Combines env `search_radius_km` (breed/temperament/season) × WP13 terrain modifier + phase (acute→near, survival ×1.5, entrenched→60km) + suspected-theft→60km + transport-risk widening; capped 80km. LLM may only tighten (post-sighting), never widen. Per-volunteer cutoff = min(volunteer willingness, case radius). Proven: friendly 4km / galgo 22.5km / theft 60km; same-spot friendly→4 vs theft→9 vols. Later nuance: a "small dog in market" ~1km could key off size + dense human_density.
