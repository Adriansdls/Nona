...
- `$FINAL_REPORT_PATH` — the polished final report from step 15
...
prompt: |
  ...
  $FINAL_REPORT_PATH has been drafted (step 10),
  ...
  - recommendations_path: $ARTIFACT_DIR/readability-recommendations.json
...
1. **Read `$ARTIFACT_DIR/readability-recommendations.json`.**
...
1. Use the Edit tool on `$FINAL_REPORT_PATH`
...
Write `$ARTIFACT_DIR/readability-decisions.json` with the orchestrator's decisions:
...
- `$ARTIFACT_DIR/readability-recommendations.json` exists
- `$ARTIFACT_DIR/readability-decisions.json` exists with at least one entry in `applied` or all `skipped`
- `$FINAL_REPORT_PATH` reflects the applied recommendations
