...
- `$ARTIFACT_DIR/prompt-decomposition.json` — atomic items, sub-questions
- `$TEMP_DIR/contradiction-graph.json` — ranked fight clusters (if step 3 ran)
- `$TEMP_DIR/coverage-gaps.md` — which atomic items have weak coverage
...
1. **Spawn 2 `hyperresearch-loci-analyst` subagents in parallel** (ONE message, both Task calls). Both read the same width corpus but return independently.
   ...
   **Spawn template:**
   ...
   prompt: |
     ...
     $TEMP_DIR/contradiction-graph.json. After you and the other
     ...
     - output_path: $ARTIFACT_DIR/loci-a.json (or $ARTIFACT_DIR/loci-b.json)
   ```
...
5. **Write scored loci to `$ARTIFACT_DIR/loci.json`.** Schema:
   ```json
   {
     "loci": [
       {
...
- `$ARTIFACT_DIR/loci.json` exists with at least 1 locus (or both analysts justified skip with `skip_loci`)
